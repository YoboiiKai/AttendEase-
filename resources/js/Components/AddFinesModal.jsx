...
import { X, Scale, Users, PhilippinePeso, BookOpenText } from 'lucide-react'
...
export default function AddFines({ isOpen, onClose, onFinesUpdate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('api/admin/fines', formData, {
        headers: { 
      ...
      if (response.data.success) {
        Swal.fire('Success', 'Fines added successfully', 'success');
        onClose(); 
        ...
        Swal.fire(
          'Error',
          response.data.message || 'Failed to add fines',
          'error'
        );
      }
    } catch (error) {
      ...
      if (error.response?.data?.errors) {
        ...
        Swal.fire(
          'Error',
          error.response?.data?.message || 'Failed to add fines',
          'error'
        );
      }
    }
